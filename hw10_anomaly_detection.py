...
def pca_after(x,num):
    ...
    #plt.imshow(y_reconstructed[num])  
...
class conv_autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout2d(),
            nn.Conv2d(6, 12, 4, stride=2, padding=1),            # [batch, 12, 16, 16]
            nn.BatchNorm2d(12),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm2d(12),
            nn.ReLU(),
            nn.ConvTranspose2d(12, 6, 4, stride=2, padding=1),# [batch, 3, 32, 32]
            nn.BatchNorm2d(6),
            nn.ReLU(),
            nn.Dropout2d(),
...
if task == 'ae':
    ...
    model = torch.load('best_model_{}_last.pth'.format(model_type), map_location='cuda')
    ...
    with open(sys.argv[2], 'w') as f:
        ...
        for i in range(len(y_pred)):
            f.write('{},{}\n'.format(i+1, y_pred[i]))
            '''
    #anomality = knn_after(reconstructed)
    #anomality = np.sqrt(np.sum(np.square(reconstructed - y).reshape(len(y), -1), axis=1))
    # for i in range(9):
    #     y_pred = anomality[i]
    #     with open('prediction_last_cnn_knn{}.csv'.format(i), 'w') as f:
    #         f.write('id,anomaly\n')
    #         for i in range(len(y_pred)):
    #             f.write('{},{}\n'.format(i+1, y_pred[i]))
    # score = roc_auc_score(y_label, y_pred, average='micro')
